...
function expandShorthand(
  ...
  if (parts.length === 2) {
    ...
    return v === null || h === null
      ? null
      : { [axisProp.vertical]: v, [axisProp.horizontal]: h };
  }
  return null;
}
...
function expandBoxShadow(
  value: string,
): { shadowColor: string; shadowOffset: { width: number; height: number }; shadowOpacity: number; shadowRadius: number; elevation: number } | null {
  ...
  const re = new RegExp(
    `^${num}\\s+${num}\\s+${num}\\s+(#[0-9a-fA-F]+|rgba?\\([^)]+\\))$`,
  );
