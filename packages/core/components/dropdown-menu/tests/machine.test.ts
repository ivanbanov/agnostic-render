...
const ctx = (m: ReturnType<typeof makeMachine>) =>
  m.getContext() as DropdownMenuContext;
...
describe("item activation", () => {
  it("activating a regular item closes the menu (closeOnSelect default)", () => {
    const m = makeMachine({ defaultOpen: true });
    api(m).getItem(ITEMS[0]).handlers.onPress?.(undefined as never);
    expect(m.getState()).toBe("idle");
  });
  ...
  it("checkbox items keep the menu open by default", () => {
    ...
    api(m, [checkbox]).getItem(checkbox).handlers.onPress?.(undefined as never);
    expect(m.getState()).toBe("open");
  });
  ...
  it("onSelect preventDefault cancels the close", () => {
    ...
    const m = makeMachine({ defaultOpen: true });
    api(m, [item]).getItem(item).handlers.onPress?.(undefined as never);
...
describe("focusTrap", () => {
  ...
  it("reflects the resolved prop when true", () => {
    expect(api(makeMachine({ defaultOpen: true, focusTrap: true })).focusTrap).toBe(
      true,
    );
  });
...
describe("connect accessibility surface", () => {
  ...
  it("items take the right role per kind and are not tab stops", () => {
    ...
    expect(a.getItem({ value: "b", kind: "checkbox" }).attrs.role).toBe(
      "menuitemcheckbox",
    );
    expect(a.getItem({ value: "c", kind: "radio" }).attrs.role).toBe(
      "menuitemradio",
    );
