/**
 * Behavior DSL — a Zag-shaped library with NO substrate assumptions.
 *
 * A behavior config is data: states, events, transitions, named guards/actions/effects.
...
export interface BehaviorConfig<TContext, TProps = Record<string, unknown>> {
...
export interface Behavior<TContext, TProps = Record<string, unknown>> {
  getState: () => string;
  getContext: () => TContext;
  getProps: () => TProps;
  /**
