...
/**
 * Create a substrate-agnostic behavior instance.
 *
 * Pure data in, pure data out. No DOM, no React, no timers tied to a host.
 * Effects can use `setTimeout` etc. — that's runtime, not a substrate.
 */
export function createBehavior<TContext extends object, TProps extends object>(
  ...
  // Monotonic counter bumped on every notification. Subscribers (React's
  // useSyncExternalStore, etc.) read this as a cheap "did anything change?"
  // snapshot — Number === Number is allocation-free.
  let version = 0;
  ...
  const runActions = (names: string[] | undefined, event: EventObject) => {
    ...
    for (const name of names) {
      ...
      if (!fn) {
        console.warn(`[behavior] no action "${name}"`);
        continue;
      }
      fn(baseParams(event));
    }
  ...
  const checkGuard = (
    ...
    if (!fn) {
      console.warn(`[behavior] no guard "${name}"`);
      return false;
    }
  ...
  const runEffects = (stateName: string) => {
    ...
    for (const name of effectNames) {
      ...
      if (!fn) {
        console.warn(`[behavior] no effect "${name}"`);
        continue;
      }
