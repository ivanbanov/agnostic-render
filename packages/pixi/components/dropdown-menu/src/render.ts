...
function isItem(entry: DropdownMenuItemConfig): entry is DropdownMenuItemProps & { label: string } {
  return !('kind' in entry) || (entry.kind !== 'separator' && entry.kind !== 'label')
}

export function createDropdownMenu(options: CreateDropdownMenuOptions): DropdownMenuHandle {
  ...
  const buildItemNodes = () => {
    ...
    for (const entry of itemConfigs) {
      ...
      const prefix = entry.kind === 'checkbox' && entry.checked ? '✓ ' : ''
  ...
  const attachKeyListener = () => {
    ...
    const onKey = (event: KeyboardEvent) => {
      ...
      const handler = (latest.open ? latest.parts.content.handlers : latest.parts.trigger.handlers)
  ...
  const attachOutsideListener = () => {
    ...
    removeOutsideListener = () => window.removeEventListener('pointerdown', onPointerDown)
  }
